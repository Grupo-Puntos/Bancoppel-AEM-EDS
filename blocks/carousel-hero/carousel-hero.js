...
function stopAutoplay(block) {
  if (block.autoplayTimer) {
    window.clearInterval(block.autoplayTimer);
...
function startAutoplay(block) {
  ...
  stopAutoplay(block);
  block.autoplayTimer = window.setInterval(() => {
...
function bindEvents(block) {
  ...
  // Autoplay: pause while the user hovers or focuses inside the carousel.
  startAutoplay(block);
...
function createSlide(row, slideIndex, carouselId) {
  ...
  // Promote the trailing "<p><a>…</a></p>" CTA into a standalone pill button.
  // decorateMain() only auto-adds .button when the link is alone in its cell;
  // here it shares a cell with the heading/paragraph, so we do it explicitly.
  ...
  if (content) {
    ...
    if (ctaPara) {
      const link = ctaPara.firstElementChild;
      link.classList.add('button');
      if (!link.title) link.title = link.textContent.trim();
      ctaPara.replaceWith(link);
    }
  }
